...
import { computed as d, inject as x, input as I } from "@angular/core";
import { param as G } from "@ng-internal/url";
import { resource as L } from "@angular/core";
...
var c = class e {
    list() { return L({ loader: () => Promise.resolve(b(g)) }); }
    byKey(a) { return L({ params: () => a() ?? void 0, loader: ({ params: r }) => { let n = g.find(l => l.key === r); return Promise.resolve(n ? u(n) : void 0); } }); }
...
var m = class e {
    ...
    routeParams = I({});
    category = d(() => G(this.routeParams(), "category"));
    categoriesResource = this.categoryHttp.list();
    categories = d(() => this.categoriesResource.value() ?? []);
    activeCategory = d(() => { let a = this.category(); return a ? this.categories().find(r => r.key === a) : void 0; });
    title = d(() => this.activeCategory()?.name ?? "All Machines");
    products = d(() => { let a = this.activeCategory(); return (a ? [...a.products] : this.categories().flatMap(n => n.products)).sort((n, l) => l.startPrice - n.startPrice); });
    filters = d(() => { let a = this.category(), r = this.activeCategory(); return [{ link: { intent: "explore.products" }, name: "All", active: !r }, ...this.categories().map(n => ({ link: { intent: "explore.products.category", params: { category: n.key } }, name: n.name, active: n.key === a }))]; });
    static \u0275fac = function (r) { return new (r || e); };
...
var M = "mfe-category";
function Ae(e, a) { return f(this, null, function* () { let r = yield v(e, a); customElements.get(M) || customElements.define(M, $(m, { injector: r })); }); }
export { Ae as bootstrap };
